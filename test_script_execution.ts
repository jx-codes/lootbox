...
async function testScriptExecution() {
  console.log("🚀 Testing Script Execution via WebSocket\n");
...
console.log("🎮 Starting Pokemon Team Analysis...");
...
`;
  ...
  console.log("📤 Sending LLM-generated script for execution:");
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.onclose = () => {
      console.log("🔌 WebSocket connection closed by server/client auto-disconnect");
    };
      ...
      try {
        ...
        if (response.id === callId) {
          ...
          if (response.error) {
            ...
            console.log("\n🎉 This demonstrates the full power:");
            console.log("• LLM writes TypeScript code using generated client");
            console.log("• Code sent as single WebSocket message");
            console.log("• Executed in sandboxed Deno environment");
            console.log("• Multiple RPC calls orchestrated within script");
            console.log("• Complex data analysis and reporting");
          }

          console.log("\n⏰ Waiting for auto-disconnect... (should happen within ~200ms)");

          // Wait a bit to see if connection auto-closes, then resolve
          setTimeout(() => {
            if (ws.readyState === WebSocket.OPEN) {
              console.log("⚠️ WebSocket still open - connection did not auto-disconnect");
              ws.close();
            } else {
              console.log("✅ WebSocket auto-disconnected successfully");
            }
            resolve(response);
    ...
    console.log("📡 WebSocket state before sending:", ws.readyState === WebSocket.OPEN ? "OPEN" : "CLOSED");
