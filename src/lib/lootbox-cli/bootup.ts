...
export function showBootup(info: BootupInfo): void {
  ...
  console.log(
    `\n  \x1b[35m▶\x1b[0m \x1b[1mlootbox\x1b[0m \x1b[2mv${VERSION}\x1b[0m\n`
  );

  console.log(
    `  \x1b[32m➜\x1b[0m  Local:   \x1b[36mhttp://localhost:\x1b[1m${port}\x1b[0m`
  );
  console.log(
    `  \x1b[32m➜\x1b[0m  WebSocket: \x1b[36mws://localhost:\x1b[1m${port}\x1b[36m/ws\x1b[0m`
  );
  ...
  console.log(
    `  \x1b[90m○\x1b[0m  RPC: ${rpcFunctions.length} function${
      rpcFunctions.length !== 1 ? "s" : ""
    }`
  );
  console.log(`  \x1b[90m○\x1b[0m  Tools: \x1b[2m${toolsDir}\x1b[0m\n`);

  console.log(
    `  \x1b[1mready\x1b[0m in \x1b[1m${Math.floor(
      performance.now()
    )}ms\x1b[0m\n`
  );
}
